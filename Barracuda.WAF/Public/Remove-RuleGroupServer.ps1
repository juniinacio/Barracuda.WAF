...
#>
function Remove-RuleGroupServer {
    [CmdletBinding()]
    [Alias()]
    ...
    process {
        foreach ($name in $WebServerName) {
            try {
                Invoke-API -Path ('/restapi/v3/services/{0}/content-rules/{1}/content-rule-servers/{2}' -f $WebApplicationName, $RuleGroupName, $WebServerName) -Method Delete
            } catch {
